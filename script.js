...
if (list) {
  ...
  const render = () => {
    ...
    list.innerHTML = items.length ? items.map((ep,index) => `<article class="library-card"><div class="cover cover-${index%4}"><small>PODCAST</small><strong>${ep.n}</strong><i>menopausa<br>não tem cor</i></div><button class="card-play" data-select="${ep.id}" aria-label="Selecionar ${ep.title}">▶</button><div class="card-copy"><div><span>${ep.theme}</span><span>${ep.duration}</span></div><h2>${ep.title}</h2><p>${ep.description}</p><small>${ep.guest}</small></div><div class="card-actions"><button class="fav ${favorites.includes(ep.id)?"active":""}" data-favorite="${ep.id}" aria-label="Favoritar episódio">♥</button><button ${ep.audio?`data-download="${ep.id}"`:"disabled"} title="Disponível quando o áudio for publicado">↓</button></div></article>`).join("") : `<div class="empty-state"><b>♡</b><h2>Nenhum episódio por aqui</h2><p>Tente outra busca ou favorite uma conversa para encontrá-la depois.</p></div>`;
  };
